window.utilCg = (function () {
    ...
    async function populateClerps(nodes, visState) {
        if (!visState.clerps) visState.clerps = new Map();

        const clerpsToFetch = nodes.filter(d =>
            d.feature_type.includes('transcoder') &&
            !visState.clerps.has(d.feature)
        );

        const fetchPromises = clerpsToFetch.map(async (d) => {
            const url = "https://www.neuronpedia.org/api/feature/gemma-2-2b/" + d.layer + "-gemmascope-transcoder-16k/" + d.feature;

            try {
                const resp = await fetch(url);
                const json = await resp.json();

                if (json && Array.isArray(json.explanations) && json.explanations.length > 0) {
                    const explanation = json.explanations[0].description;
                    visState.clerps.set(d.feature, explanation);
                    d.clerp = explanation;
                } else {
                    const fallback = "No explanation found.";
                    visState.clerps.set(d.feature, fallback);
                    d.clerp = fallback;
                }
            } catch (err) {
                console.error(`Failed to fetch for ${d.feature}:`, err);
    ...
    async function formatData(data, visState) {
        ...
        //await populateClerps(nodes, visState);
    ...
    function initBcSync({visState, renderAll}) {
        ...
        renderAll.pinnedIds.fns.push(ev => {
            if (!ev?.skipBroadcast) broadcastState()
        })
        renderAll.hiddenIds.fns.push(ev => {
            if (!ev?.skipBroadcast) broadcastState()
        })
        renderAll.clickedId.fns.push(ev => {
            if (!ev?.skipBroadcast) broadcastState()
        })
        renderAll.hoveredId.fns.push(ev => {
            if (!ev?.skipBroadcast) broadcastState()
        })
