window.initCgLinkGraph = function ({visState, renderAll, data, cgSel}) {
    ...
    // set up dom
    // Text nodes (only if no overlap)
    var nodeSel = c.svg.appendMany('g.node', nodes)
        .translate(d => d.pos)

    nodeSel.filter(d => !d.has_overlap)   // normal case: text
        .append('text')
    ...
    // Diamond shape (if has_overlap)
    let diamondSize = 4 // adjust size
    nodeSel.filter(d => d.has_overlap)
        .append('path')
        .at({
            d: d3.symbol().type(d3.symbolDiamond).size(diamondSize * diamondSize)(),
            fill: d => d.nodeColor,
            stroke: '#000',
            strokeWidth: .5
        })
